...
#'
#' @description This function runs the Falcó algorithm for a given dataset. In the `falco.R` file we can find the kernel function, `K`; the `f_hat_h` function, for the estimation of the probability of belonging to a class; and the `falco` function, which is the main function that runs the Falcó algorithm.
#'
...
get_rho_d = function(
  ...
  new_Data %<>%
    ...
    map_if(is.factor, function(v) {
      # Ya no contamos con variables numéricas, sino que tenemos solo factores
      l_v = levels(v) %>% length
      v %>%
        map(function(x) {
  ...
  for (i in 1:n_y_classes) {
    C[[i]] = Data %>%
      filter(!!sym(new_objective[i]) == 1) %>%
      select(-starts_with(objective_var))
  }
...
get_C_tilde_polar = function(
  ...
  if (rank_rho > 2) {
    stop(
      "El rango de rho_d es mayor a 2, y actualmente admitimos clasificación binaria únicamente"
    )
  }

  ## Coordenadas esféricas en R^r
  # Pasamos ahora cada vector de D_tilde a coordenadas esféricas
  train_C_tilde_polar <- train_C_tilde %>%
    map(
      ~ .x %>%
        as_tibble() %>%
  ...
  test_C_tilde_polar <- test_C_tilde %>%
    map(
      ~ .x %>%
        as_tibble() %>%
        set_names(paste0("V_", 1:ncol(.))) %>% # Esta V realmente representa X_tilde_r
  ...
  ff.test = fasano.franceschini.test::fasano.franceschini.test(
    train_C_tilde_polar[[1]],
    train_C_tilde_polar[[2]]
  ) # Funciona, pues es multivariante
  if (!(ff.test$p.value < 0.05)) {
    warning(
      "No hay evidencia significativa de diferencia entre las distribuciones. Posiblemente nada funcione\n"
    )
  } else {
...
dmm = function(
  ...
  C_tilde_polar <- get_C_tilde_polar(
    Data,
  ...
  classify = function(x, D_tilde) {
    ...
    new_class = clases %>%
      map_dbl(~ f_hat_h(D_tilde, x, clase = .x)) %>%
        ...
        ~ .x %>%
          mutate(
            .pred = map_dbl(
              .x = pmap_dbl(
                select(., r, phi_1),
                ~ c(...) %>% classify(D_tilde = D_tilde_train)
              ),
              .f = ~.x
            ),
            .truth = .y
