...
Buscamos en este documento contrastar el método de la matriz cuántica de dm con otros modelos clásicos de clasificación, como los árboles de decisión o el PCA. Para ello, aplicaremos estos 3 métodos al conjunto de datos SPN (Nódulo Pulmonar Solitario).
...
En todos los casos luego hemos usado Data_train para preparar la receta. Hecho esto hemos entrenado los modelos mediante una búsqueda en rejilla —cosa que en este caso no hace falta—, para luego estimar el error mediante `tune_grid()` y `train_resamples()`. Recordemos también que hemos definido `my_metrics()` para recuperar el error. Para seguir el mismo procedimiento con nuestra función predictiva `dm()` tenemos que hacerlo más paso a paso, como sigue:
...
```{r 2025-02-SPN-13 }
source(here("experiments/SPN_code/dm/dm.R"))
```
...
```{r 2025-02-SPN-14 }
source(here("experiments/SPN_code/dm/plot_dm.R"))
dm_plot
```
...
Hemos generado el objeto `dm_metrics`, que contiene las métricas de error para cada uno de los experimentos realizados. Ahora, vamos a comparar estos resultados con los obtenidos en el apartado anterior, para cada uno de los casos de estudio.
...
Queremos ahora **evaluar sobre el conjunto `test`**. Para ello, finalizamos el ajuste con el workflow general ML y obtenemos las predicciones, y análogamente para el caso dm.
